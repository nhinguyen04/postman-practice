...
async function workout(totalTime) {
  try {
    const stretchRemain = await stretch(totalTime);
    const runRemain = await runOnTreadmill(stretchRemain);
    const liftRemain = await liftWeights(runRemain);
    console.log(`done working out with ${liftRemain/1000} seconds left`);
  } catch (error) {
